...
func (r *Runner) assertBody(_ interfaces.ExecutionContext, assert *tests.Assert, _ *http.Response, body []byte) error {
	if assert.Exists {
		if len(body) == 0 {
			return fmt.Errorf("assertion failed, expected not null body")
		}
	...
	if assert.Equals != nil {
		...
		if !reflect.DeepEqual(body, expected) {
			return fmt.Errorf("assert failed, expected body %v to equal %v", string(body), expected)
		}
		return nil
	}
	...
	if assert.Contains != "" {
		if !bytes.Contains(body, []byte(assert.Contains)) {
			return fmt.Errorf("assertion failed, expected %v in body", assert.Contains)
		}
...
func (r *Runner) assertHeaders(_ interfaces.ExecutionContext, assert *tests.Assert, resp *http.Response) error {
	if assert.Equals != nil {
		if equals, ok := assert.Equals.(map[string]any); ok {
			return fmt.Errorf("assertion failed, expected map assertion got %v", assert.Equals)
		} else {
			for key, expectedVal := range equals {
				actualVal := resp.Header.Get(key)
				if fmt.Sprintf("%v", expectedVal) != actualVal {
					return fmt.Errorf("assertion failed, expected header value %v, got %v", expectedVal, actualVal)
				}
			}
		}
	}
	...
	if assert.Contains != "" {
		...
		if !found {
			return fmt.Errorf("assertion failed, expected header %v but not found", assert.Contains)
		}
	}

	if assert.Exists {
		if len(resp.Header) == 0 {
			return fmt.Errorf("assertion failed, expected some headers in response")
		}
	}
